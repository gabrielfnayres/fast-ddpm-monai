...
@NoiseSchedules.add_def("alpha", "Task Specific Alpha Schedule")
def _alpha_task_schedule(num_train_timesteps, beta_function):
    beta = torch.cat([torch.zeros(1).to(beta_function.device), beta_function], dim=0)

    a = (1 - beta).cumprod(dim=0).index_select(0, num_train_timesteps + 1).view(-1,1,1,1)
    return a
